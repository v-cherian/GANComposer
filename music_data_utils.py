...
class MusicDataLoader(object):
        ...
        if not datadir is None:
            ...
            if redo_split:
                ...
                if self.composers is None:
                    self.saved_songs_path = os.path.join(
                        self.datadir, 'saved_songs', 'all_composers.txt')
                else:
                    self.saved_songs_path = os.path.join(
                        self.datadir, 'saved_songs', str(len(self.composers)) + '_composers.txt')
                    print('WARNING: using previous songs state at ' +
                          self.saved_songs_path)
                    print(
                        'use --redo_split if this composer list is different from the one used for above state')

                if not os.path.exists(self.saved_songs_path):
                    print(
                        '--redo_split flag excluded but saved variable not found at ' + self.saved_songs_path)
                    print('Redoing split')
                    self.read_data(pace_events)
                else:
    ...
    def read_saved_songs(self):
        print('Using saved songs at ' + self.saved_songs_path)
        with open(self.saved_songs_path, 'rb') as fp:
    ...
    def read_data(self, pace_events):
        ...
        csv_path = os.path.join(
            self.datadir, os.path.basename(self.datadir)+".csv")
        ...
        df = pd.read_csv(csv_path, usecols=[
                         'canonical_composer', 'split', 'midi_filename'])
        ...
        for row_index, row in df.iterrows():
            ...
            if os.path.isfile(os.path.join(self.datadir, row['midi_filename'])):
                ...
                midi_path = os.path.join(train_path, os.path.basename(
                    row['midi_filename']))  # reassign midi_path
                if row['split'] == 'train':
                    midi_path = os.path.join(train_path, os.path.basename(
                        row['midi_filename']))  # reassign midi_path
                    self.songs['train'].append(
                        [midi_path, composer, song_data])
                elif row['split'] == 'validation':
                    midi_path = os.path.join(validation_path, os.path.basename(
                        row['midi_filename']))  # reassign midi_path
                    self.songs['validation'].append(
                        [midi_path, composer, song_data])
                elif row['split'] == 'test':
                    midi_path = os.path.join(test_path, os.path.basename(
                        row['midi_filename']))  # reassign midi_path
                    self.songs['test'].append([midi_path, composer, song_data])

                shutil.copyfile(os.path.join(
                    self.datadir, row['midi_filename']), midi_path)
        ...
        self.saved_songs_path = os.path.join(self.datadir, 'saved_songs')

        if not os.path.exists(self.saved_songs_path):
            os.makedirs(self.saved_songs_path)

        if composers_specified:
            self.saved_songs_path = os.path.join(
                self.saved_songs_path, str(len(self.composers))+'_composers.txt')
        else:
            self.saved_songs_path = os.path.join(
                self.saved_songs_path, 'all_composers.txt')

        print('Saving songs state in ' + self.saved_songs_path)
        with open(self.saved_songs_path, 'wb') as fp:
    ...
    def read_one_file(self, path, pace_events):
        ...
        input_ticks_per_output_tick = ticks_per_quarter_note / \
        ...
        for track in midi_pattern:
                ...
                if type(event) == midi.events.SetTempoEvent:
                    ...
                    for e in not_closed_notes:
                        if event.data[0] == e[TONE]:
                            event_abs_tick = float(
                                event.tick+last_event_input_tick)/input_ticks_per_output_tick
            ...
            for e in not_closed_notes:
                ...
                e[LENGTH] = float(ticks_per_quarter_note) / \
        ...
        if (pace_events):
            ...
            while pace_tick < song_tick_length:
                ...
                pace_tick += float(ticks_per_quarter_note) / \
                    input_ticks_per_output_tick
            song_data.sort(key=lambda e: e[BEGIN_TICK])
                        ...
                        for simultaneous in range(1, self.tones_per_cell):
                            ...
                            if batch[s][SONG_DATA][n+simultaneous][BEGIN_TICK]-batch[s][SONG_DATA][n][BEGIN_TICK] == 0:
                                ...
                                event[offset+LENGTH] = batch[s][SONG_DATA][n +
                                                                           simultaneous][LENGTH]
                                event[offset+TONE] = batch[s][SONG_DATA][n +
                                                                         simultaneous][TONE]
    ...
    def get_midi_pattern(self, song_data, bpm, normalized=True):
        ...
        # Tempo:
        # Multiply with output_ticks_pr_input_tick for output ticks.
        midi_pattern = midi.Pattern([], resolution=int(
            self.output_ticks_per_quarter_note))
        ...
        cur_track.append(midi.EndOfTrackEvent(
            tick=int(self.output_ticks_per_quarter_note)))
